...
class GANLoss(nn.Module):
    ...
    def forward(self, fake_scores, real_scores=None):
        if real_scores is None:
            ...
            real_label = torch.ones(fake_scores.shape[0]).to(device_glob)
            fake_label = torch.zeros(fake_scores.shape[0]).to(device_glob)

            if self.loss_type == 'non_saturating':
                # loss = -torch.mean(torch.log(fake_scores + 1e-8))
                # loss = F.binary_cross_entropy(fake_scores, real_label)
            ...
            real_label = torch.ones(real_scores.shape[0]).to(device_glob)
            fake_label = torch.zeros(fake_scores.shape[0]).to(device_glob)

            if self.loss_type == 'non_saturating':
                loss = F.binary_cross_entropy_with_logits(real_scores, real_label) + F.binary_cross_entropy_with_logits(fake_scores, fake_label)
                # loss = F.binary_cross_entropy(real_scores, real_label) + F.binary_cross_entropy(fake_scores, fake_label)
                # loss = -torch.mean(torch.log(real_scores + 1e-8) + torch.log(1 - fake_scores + 1e-8))

            if self.loss_type == 'hinge':
                loss = (-torch.mean(torch.minimum(fake_label, -1 + real_scores)) - torch.mean(torch.minimum(fake_label, -1 - fake_scores)))
...
class ValLoss(nn.Module):
    ...
    def calc_fid(real_features, fake_features):
        ...
        cov_real = np.cov(real_features)
        cov_fake = np.cov(fake_features)

        mean_real = np.mean(real_features, axis=1)
        mean_fake = np.mean(fake_features, axis=1)
        
        outputs = np.linalg.norm(mean_real - mean_fake, 2) ** 2 + np.trace(cov_real + cov_fake - 2 * linalg.sqrtm(cov_real @ cov_fake))

        return outputs  # TODO (2 points)

    @staticmethod
    def calc_is(fake_probs):
        marginal_dist = np.sum(fake_probs, axis=0) / fake_probs.shape[0]

        outputs = fake_probs.copy()
        for i in range(fake_probs.shape[1]):
            outputs[:, i] = outputs[:, i] / marginal_dist[i]
        outputs = fake_probs * np.log(outputs)
        outputs = np.sum(outputs, axis=1)

        # outputs = []
        # for i in range(fake_probs.shape[0]):
        #     for k in range(fake_probs.shape[1]):
        #         outputs.append(fake_probs[i, k] * np.log(fake_probs[i, k] / marginal_dist[i]))

        return np.exp(np.mean(outputs)) # TODO (2 points)
