...
class GAN(pl.LightningModule):
    def __init__(
        ...
        if self.truncation_trick:
            ...
            mean = np.zeros((len(self.train_dataset), self.noise_channels))
            std = np.ones((len(self.train_dataset), self.noise_channels))
            bounds = np.array([-0.5, 0.5])
            self.val_noise.data = torch.from_numpy(stats.truncnorm.rvs((bounds[0] - mean) / std, (bounds[1] - mean) / std, loc=mean, scale=std))
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        noise = torch.randn(labels.shape[0], self.noise_channels).to(device_glob).detach()
        
        if optimizer_idx == 0:
            pred_gen = self.gen(noise, labels)
            pred_dis = self.dis(pred_gen, labels)

            loss = self.loss(pred_dis)
            
        elif optimizer_idx == 1:
            pred_gen = self.gen(noise, labels).detach()
            pred_dis = self.dis(pred_gen, labels)

            loss = self.loss(pred_dis, self.dis(imgs, labels))
            # loss = self.loss(pred_dis + self.eps, torch.sigmoid(self.dis(imgs, labels)) + self.eps)
