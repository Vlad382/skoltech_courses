...
class CustomMultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        in_proj_res = self.in_proj(qkv)
        query, key, value = torch.split(in_proj_res, self.embed_dim, dim=2)
        result = torch.Tensor()

        for i in range(self.num_heads):
            query_i = query[:, :, i*self.head_dim: (i+1)*self.head_dim]
            key_i = key[:, :, i*self.head_dim: (i+1)*self.head_dim]
            value_i = value[:, :, i*self.head_dim: (i+1)*self.head_dim]

            sqrt_d_k = torch.Tensor([key_i.shape[-1]]).sqrt().item()
            key_i_transpose = torch.transpose(key_i, 1, 2)

            attention_i = torch.bmm(torch.nn.Softmax(2)(torch.bmm(query_i, key_i_transpose) / sqrt_d_k), value_i)
            attention_i = self.attention_dropout(attention_i)

            result = torch.cat([result, attention_i], dim=2)
...
class StepLRWithWarmup(torch.optim.lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        ...
        if (self.last_epoch == 0):
            # print([self.warmup_lr_init for _ in self.optimizer.param_groups])
            return [self.warmup_lr_init for _ in self.optimizer.param_groups]
        
        while (self.last_epoch > 0) & (self.last_epoch < self.warmup_epochs):
            # print([self.last_epoch * warmup_incr for _ in self.optimizer.param_groups])
            return [self.warmup_lr_init + self.last_epoch * warmup_incr for _ in self.optimizer.param_groups]
        
        if self.last_epoch == self.warmup_epochs:
            # print([self.base_lrs for _ in self.optimizer.param_groups][0])
